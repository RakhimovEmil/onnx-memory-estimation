...
def load_nodes(model: onnx.ModelProto, placeholder_names: Set[str]) -> Tuple[Dict[str, ValueType], List[NodeWrapper]]:
    ...
    edges: Dict[str, Tuple[Optional[str], List[str]]] = dict()

    for node in model.graph.node:
        if not hasattr(node, 'domain') and node.op_type == 'Constant':
            continue
        node_wrapped = _load_regular_node(node)

        # храню лишнюю память. можно не хранить НЕ мутабельные ребра
        for input_name in node_wrapped.input_names:
            if input_name in edges:
                edges[input_name][1].append(node.name)
            else:
                edges[input_name] = (None, [node.name])
        for output_name in node_wrapped.output_names:
            edges[output_name] = (node.name, [])
        nodes.append(node_wrapped)
...
def get_ancestors_max_asap(node: NodeWrapper, edges: Dict[str, Tuple[Optional[str], List[str]]], asap: Dict[str, int]) -> int:
    max_asap = -1
    for edge_name in node.input_names:
        ancestor_name = edges[edge_name][0]
        if ancestor_name is None or ancestor_name not in asap:
            max_asap = max(max_asap, 0)
        else:
            max_asap = max(max_asap, asap[ancestor_name])
    return max_asap
...
def find_min_alap(
    ...
    return min((alap.get(child, last_asap) for child in child_nodes), default=last_asap)

def get_nodes_alap(nodes: List[NodeWrapper], edges: Dict[str, Tuple[Optional[str], List[str]]], last_asap: int) -> Dict[str, int]:
    alap: Dict[str, int] = {nodes[-1].name: last_asap}

    for node in nodes[::-1]:
        if node.name in alap:
            continue
        
        min_alap = last_asap # local +inf
        for edge_name in node.output_names:
            child_nodes = edges[edge_name][1]
            min_alap = min(
                min_alap,
                find_min_alap(child_nodes, alap, last_asap)
            )
        alap[node.name] = min_alap - 1
    return alap
